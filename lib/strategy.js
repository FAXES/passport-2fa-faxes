...
function Strategy(options, verifyUsernameAndPassword, verifyTotpCode) {
    ...
    this._totpOptional = options.totpOptional || false;
    this._removeUserKeys = options.removeUserKeys || [];
...
Strategy.prototype.authenticate = function (req, options) {
    ...
    var firstStepAuth = new Promise(function (resolve, reject) {
        ...
        var verify = function (error, user, info) {
            if(self._removeUserKeys) {
                for (let i = 0; i < self._removeUserKeys.length; i++) {
                    const e = self._removeUserKeys[i];
                    delete user[e];
                }
            }
            if(info == 'UNREG') return resolve(user);
            if (error) {
                return reject({
                    error: true,
                    message: error
